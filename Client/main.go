	...
	for{
		...
		switch sqlType {
			...
			case SELECT_MULTI: //select *(c1,c2,...) from tableName1,tableName2 where ...
				...
				selectFields := strings.Split(matches[1], ",")//属性名数组
...
func showTables() []string{
	resp, err := http.Get("http://" + masterIP + ":8080/table/show")
	if err != nil {
...
func newTable(tableName string) string {
	resp, err := http.Get("http://" + masterIP + ":8080/table/new?tableName=" + tableName)
	if err != nil {
		fmt.Println("Request failed:", err)
		os.Exit(1)
	}
...
func deleteTable (tableName string) string {
	resp, err := http.Get("http://" + masterIP + ":8080/table/delete?tableName=" + tableName)
	if err != nil {
		fmt.Println("Request failed:", err)
		os.Exit(1)
	}
...
func getTableIPs(tableNames []string) {
	...
	resp, err := http.Post("http://" + masterIP + ":8080/table/query", "application/json", bytes.NewReader(jsonBody))
	if err != nil {
		fmt.Println("Request failed:", err)
		os.Exit(1)
	}
...
func readSQL(tableName string, sql string) (cols []string, rows [][]string) {
	...
	resp, err := http.Post("http://" + tableIPs[tableName] + ":8080/sql/read", "application/json", bytes.NewReader(jsonBody))
	if err != nil {
		fmt.Println("Request failed:", err)
		os.Exit(1)
	}
	...
	if !response.Success {
		if response.ErrCode == "403" {
			fmt.Println("Request failed:", "Access denied")
...
func writeSQL(tableName string, sql string) bool {
	...
	resp, err := http.Post("http://" + tableIPs[tableName] + ":8080/sql/write", "application/json", bytes.NewReader(jsonBody))
	if err != nil {
		fmt.Println("Request failed:", err)
		os.Exit(1)
	}
...
func regionCreateTable(tableName string, sql string) bool {
	...
	resp, err := http.Post("http://" + tableIPs[tableName] + ":8080/sql/create", "application/json", bytes.NewReader(jsonBody))
	if err != nil {
		fmt.Println("Request failed:", err)
		os.Exit(1)
	}
...
func regionDeleteTable(tableName string, sql string) bool {
	...
	resp, err := http.Post("http://" + tableIPs[tableName] + ":8080/sql/delete", "application/json", bytes.NewReader(jsonBody))
	if err != nil {
		fmt.Println("Request failed:", err)
		os.Exit(1)
	}
